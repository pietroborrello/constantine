//===- CustomLoopIdiomRecognize.cpp - Loop idiom recognition --------------------===//
//
...
#define DEBUG_TYPE "custom-loop-idiom"
...
static cl::opt<bool>
    DisableLIRPAll("disable-" DEBUG_TYPE "-all",
                   cl::desc("Options to disable Loop Idiom Recognize Pass."), cl::init(false),
                   cl::ReallyHidden);

static cl::opt<bool>
    DisableLIRPMemset("disable-" DEBUG_TYPE "-memset",
                      cl::desc("Proceed with loop idiom recognize pass, but do "
                               "not convert loop(s) to memset."), cl::init(false),
                      cl::ReallyHidden);

static cl::opt<bool>
    DisableLIRPMemcpy("disable-" DEBUG_TYPE "-memcpy",
                      cl::desc("Proceed with loop idiom recognize pass, but do "
                               "not convert loop(s) to memcpy."), cl::init(false),
                      cl::ReallyHidden);

static cl::opt<bool> UseCustomLIRCodeSizeHeurs(
    "use-custom-lir-code-size-heurs",
...
class CustomLoopIdiomRecognizePass : public PassInfoMixin<CustomLoopIdiomRecognizePass> {
public:
  PreservedAnalyses run(Loop &L, LoopAnalysisManager &AM,
                        LoopStandardAnalysisResults &AR, LPMUpdater &U);
};

class CustomLoopIdiomRecognize {
...
public:
  explicit CustomLoopIdiomRecognize(AliasAnalysis *AA, DominatorTree *DT,
...
class CustomLoopIdiomRecognizeLegacyPass : public LoopPass {
public:
  static char ID;

  explicit CustomLoopIdiomRecognizeLegacyPass() : LoopPass(ID) {
    // initializeCustomLoopIdiomRecognizeLegacyPassPass(
  ...
  bool runOnLoop(Loop *L, LPPassManager &LPM) override {
    ...
    CustomLoopIdiomRecognize LIR(AA, DT, LI, SE, TLI, TTI, DL, ORE);
...
char CustomLoopIdiomRecognizeLegacyPass::ID = 0;

PreservedAnalyses CustomLoopIdiomRecognizePass::run(Loop &L, LoopAnalysisManager &AM,
  ...
  if (!ORE)
    report_fatal_error(
        "CustomLoopIdiomRecognizePass: OptimizationRemarkEmitterAnalysis not cached "
        "at a higher level");

  CustomLoopIdiomRecognize LIR(&AR.AA, &AR.DT, &AR.LI, &AR.SE, &AR.TLI, &AR.TTI, DL,
                         *ORE);
  if (!LIR.runOnLoop(&L))
    return PreservedAnalyses::all();
...
// INITIALIZE_PASS_BEGIN(CustomLoopIdiomRecognizeLegacyPass, "custom-loop-idiom",
//                       "Recognize loop idioms", false, false)
// INITIALIZE_PASS_DEPENDENCY(LoopPass)
// INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfoWrapperPass)
// INITIALIZE_PASS_DEPENDENCY(TargetTransformInfoWrapperPass)
// INITIALIZE_PASS_END(CustomLoopIdiomRecognizeLegacyPass, "custom-loop-idiom",
//                     "Recognize loop idioms", false, false)

// Pass *llvm::createCustomLoopIdiomPass() { return new CustomLoopIdiomRecognizeLegacyPass(); }
...
//===----------------------------------------------------------------------===//
//
//          Implementation of CustomLoopIdiomRecognize
//
//===----------------------------------------------------------------------===//

bool CustomLoopIdiomRecognize::runOnLoop(Loop *L) {
  CurLoop = L;
  ...
  ApplyCodeSizeHeuristics =
      L->getHeader()->getParent()->hasOptSize() && UseCustomLIRCodeSizeHeurs;
...
bool CustomLoopIdiomRecognize::runOnCountableLoop() {
...
CustomLoopIdiomRecognize::LegalStoreKind
CustomLoopIdiomRecognize::isLegalStore(StoreInst *SI) {
  ...
  if (!UnorderedAtomic && HasMemset && SplatValue && !DisableLIRPMemset &&
      // Verify that the stored value is loop invariant.  If not, we can't
      // promote the memset.
      CurLoop->isLoopInvariant(SplatValue)) {
    // It looks like we can use SplatValue.
    return LegalStoreKind::Memset;
  } else if (!UnorderedAtomic && HasMemsetPattern && !DisableLIRPMemset &&
  ...
  if (HasMemcpy && !DisableLIRPMemcpy) {
...
void CustomLoopIdiomRecognize::collectStores(BasicBlock *BB) {
...
bool CustomLoopIdiomRecognize::runOnLoopBlock(
...
bool CustomLoopIdiomRecognize::processLoopStores(SmallVectorImpl<StoreInst *> &SL,
...
bool CustomLoopIdiomRecognize::processLoopMemSet(MemSetInst *MSI,
                                           const SCEV *BECount) {
...
bool CustomLoopIdiomRecognize::processLoopStridedStore(
  ...
  SCEVExpander Expander(*SE, *DL, "custom-loop-idiom");
...
/// for (i) A[i] = B[i];
bool CustomLoopIdiomRecognize::processLoopStoreOfLoopLoad(StoreInst *SI,
                                                    const SCEV *BECount) {
...
//
bool CustomLoopIdiomRecognize::avoidLIRForMultiBlockLoop(bool IsMemset,
                                                   bool IsLoopMemset) {
...
bool CustomLoopIdiomRecognize::runOnNoncountableLoop() {
  LLVM_DEBUG(dbgs() << DEBUG_TYPE " Scanning: F["
                    << CurLoop->getHeader()->getParent()->getName()
                    << "] Noncountable Loop %"
...
bool CustomLoopIdiomRecognize::recognizeAndInsertFFS() {
...
bool CustomLoopIdiomRecognize::recognizePopcount() {
...
void CustomLoopIdiomRecognize::transformLoopToCountable(
...
void CustomLoopIdiomRecognize::transformLoopToPopcount(BasicBlock *PreCondBB,
                                                 Instruction *CntInst,
                                                 PHINode *CntPhi, Value *Var) {
...
RegisterPass<CustomLoopIdiomRecognizeLegacyPass> MP("custom-loop-idiom", "custom LoopIdiomRecognize Pass");