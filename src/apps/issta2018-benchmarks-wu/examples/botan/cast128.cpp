...
*/
template<typename T> inline uint8_t get_byte(size_t byte_num, T input) __attribute__((always_inline))
{
...
*/
inline void store_be(uint16_t in, uint8_t out[2]) __attribute__((always_inline))
{
   out[0] = get_byte(0, in);
   out[1] = get_byte(1, in);
}

const uint32_t CAST_SBOX1[256] __attribute__((aligned(64))) = {
...
const uint32_t CAST_SBOX2[256] __attribute__((aligned(64))) = {
...
const uint32_t CAST_SBOX3[256] __attribute__((aligned(64))) = {
...
const uint32_t CAST_SBOX4[256] __attribute__((aligned(64))) = {
...
*/
inline void R1(uint32_t& L, uint32_t R, uint32_t MK, uint8_t RK) __attribute__((always_inline))
{
...
*/
inline void R2(uint32_t& L, uint32_t R, uint32_t MK, uint8_t RK) __attribute__((always_inline))
{
   uint32_t T = rotate_left(MK ^ R, RK);
   L ^= (CAST_SBOX1[get_byte(0, T)]  - CAST_SBOX2[get_byte(1, T)] +
         CAST_SBOX3[get_byte(2, T)]) ^ CAST_SBOX4[get_byte(3, T)];
...
*/
inline void R3(uint32_t& L, uint32_t R, uint32_t MK, uint8_t RK) __attribute__((always_inline))
{ 
   uint32_t T = rotate_left(MK - R, RK);
   L ^= ((CAST_SBOX1[get_byte(0, T)]  + CAST_SBOX2[get_byte(1, T)]) ^
          CAST_SBOX3[get_byte(2, T)]) - CAST_SBOX4[get_byte(3, T)];
...
* CAST-128 Encryption
*/
void encrypt_n(std::vector<uint32_t>& m_MK, std::vector<uint8_t>& m_RK, const uint8_t in[], uint8_t out[]) 
{
   uint32_t L, R;
...
* CAST-128 Decryption
*/
void decrypt_n(std::vector<uint32_t>& m_MK, std::vector<uint8_t>& m_RK, const uint8_t in[], uint8_t out[])
{
   uint32_t L, R;
   L=*((uint32_t *)(in));
   R=*((uint32_t *)(in + 4));
...
*/
inline void cast_ks(std::vector<uint32_t>& K,
                       uint32_t X[]) __attribute__((always_inline))
{
   ...
   static const uint32_t S8[256] __attribute__((aligned(64))) = {
...
*/
void key_schedule(const uint8_t key[], std::vector<uint32_t>& m_MK, std::vector<uint8_t>& m_RK)
{
   m_MK.resize(48);
   m_RK.resize(48);
...
static uint8_t in_key[32] = {116, 83, 200, 204, 227, 1, 238, 197, 106, 107, 132, 219, 90, 54, 235, 206, 148, 191, 203, 76, 47, 111, 197, 159, 155, 92, 251, 156, 54, 99, 87, 238};
