...
* MISTY1 FI Function
*/
inline uint16_t FI(uint16_t input, uint16_t key7, uint16_t key9) __attribute__((always_inline))
{
...
* MISTY1 Decryption
*/
void decrypt_n(std::vector<uint16_t> &m_DK, const uint8_t in[], uint8_t out[])
{
...
* MISTY1 Key Schedule
*/
void key_schedule(const uint8_t key[], std::vector<uint16_t> &m_EK, std::vector<uint16_t> &m_DK, size_t length)
{
   uint16_t KS[32];
...
int main(int argc, char *argv[])
{
   ...
   key_schedule(in_key, m_EK, m_DK, 16);
