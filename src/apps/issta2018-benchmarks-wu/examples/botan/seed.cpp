...
const uint32_t SEED_S0[256] __attribute__((aligned(64))) = {
...
const uint32_t SEED_S1[256] __attribute__((aligned(64))) = {
...
const uint32_t SEED_S2[256] __attribute__((aligned(64))) = {
...
const uint32_t SEED_S3[256] __attribute__((aligned(64))) = {
...
* SEED G Function
*/
inline uint32_t SEED_G(uint32_t X) __attribute__((always_inline))
{
...
* SEED Encryption
*/
void encrypt_n(std::vector<uint32_t> &m_K, const uint8_t in[], uint8_t out[])
{
...
* SEED Decryption
*/
void decrypt_n(std::vector<uint32_t> &m_K, const uint8_t in[], uint8_t out[])    
{
   uint32_t B0 = load_be<uint32_t>(in, 0);
   uint32_t B1 = load_be<uint32_t>(in, 1);
   uint32_t B2 = load_be<uint32_t>(in, 2);
...
* SEED Key Schedule
*/
void key_schedule(const uint8_t key[], std::vector<uint32_t> &m_K, size_t)
{
...
static uint8_t in_key[32] = {157, 35, 214, 148, 209, 128, 252, 252, 147, 36, 179, 230, 26, 226, 109, 176, 116, 47, 75, 166, 215, 35, 186, 247, 244, 126, 75, 96, 113, 169, 246, 85};
