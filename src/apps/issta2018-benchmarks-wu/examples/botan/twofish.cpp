...
*/
template<typename T>
inline T load_le(const uint8_t in[], size_t off) __attribute__((always_inline))
{
   in += off * sizeof(T);
   T out = 0;
   for(size_t i = 0; i != sizeof(T); ++i)
...
*/
inline void store_le(uint32_t in, uint8_t out[4]) __attribute__((always_inline))
{
   out[0] = get_byte(3, in);
   out[1] = get_byte(2, in);
   out[2] = get_byte(1, in);
...
const uint8_t Q0[256] __attribute__((aligned(64))) = {
...
const uint8_t Q1[256] __attribute__((aligned(64))) = {
...
const uint8_t RS[32] __attribute__((aligned(64))) = {
...
const uint8_t EXP_TO_POLY[256] __attribute__((aligned(64))) = {
...
const uint8_t POLY_TO_EXP[256] __attribute__((aligned(64))) = {
...
const uint32_t MDS0[256] __attribute__((aligned(64))) = {
...
const uint32_t MDS1[256] __attribute__((aligned(64))) = {
...
const uint32_t MDS2[256] __attribute__((aligned(64))) = {
...
const uint32_t MDS3[256] __attribute__((aligned(64))) = {
...
* Twofish Encryption
*/
__always_inline void encrypt_n(std::vector<uint32_t> &m_RK, std::vector<uint32_t> &m_SB, const uint8_t in[], uint8_t out[])
{
  uint32_t A, B, C, D;
...
* Twofish Decryption
*/
void decrypt_n(std::vector<uint32_t> &m_RK, std::vector<uint32_t> &m_SB, const uint8_t in[], uint8_t out[])
{
  uint32_t A, B, C, D;
...
*/
__always_inline void key_schedule(const uint8_t key[], std::vector<uint32_t> &m_SB, std::vector<uint32_t> &m_RK, size_t length)
{
   m_SB.resize(1024);
   m_RK.resize(40);
...
static uint8_t in_key[24]  __attribute__((aligned(64)))={155, 119, 96, 119, 47, 12, 241, 4, 99, 196, 222, 189, 114, 88, 175, 123, 229, 147, 24, 239, 143, 190, 125, 186};
static uint8_t in[64] = {0x00};
static uint8_t out[64] = {0};
int main(int argc, char *argv[])
{
   std::vector<uint32_t> m_SB, m_RK;
   read(0, in_key, 24);
   read(0, in, 64);
   key_schedule(in_key, m_SB, m_RK, 16);
   encrypt_n(m_RK, m_SB, in, out);
   write(1, in_key, 24);
   write(1, m_SB.data(), 1024*4);
   write(1, m_RK.data(), 40*4);
   write(1, out, 64);
   return 0;
}
