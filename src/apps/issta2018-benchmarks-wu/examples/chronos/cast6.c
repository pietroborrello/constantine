...
/*forward quad round*/
static inline __attribute__((always_inline)) void Q(uint32_t *block, uint8_t *Kr, uint32_t *Km)
{
	uint32_t I;
...
/*reverse quad round*/
static inline  __attribute__((always_inline)) void QBAR(uint32_t *block, uint8_t *Kr, uint32_t *Km)
{
	uint32_t I;
	block[3] ^= F1(block[0], Kr[3], Km[3]);
	block[0] ^= F3(block[1], Kr[2], Km[2]);
